...
########################
      ...
      "Montar-desmontar")
        ...
        echo " Inicio de rotina [`date +%d-%m-%Y.%T`] === Montar e Desmontar === Fim de rotina [`date +%d-%m-%Y.%T`] " >> $LOG
...
then
	...
	NOVO_DISP=`df | cut -d / -f 6 | tail -1 | zenity --ok-label=voltar --cancel-label=sair --title="Dispositivo Novo" \
--text="Nome do disposito" --list --column "Removível (clique sobre o dispositivo)"`
	exit 0
if [ $? != 0 ]
then
	echo " Inicio de rotina [`date +%d-%m-%Y.%T`] === Voltou para o inicio do programa ===  Fim de rotina [`date +%d-%m-%Y.%T`] " >> $LOG
